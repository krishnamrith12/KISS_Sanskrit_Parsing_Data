...
class LM_LSTM_CRF(nn.Module):
    ...
    def __init__(self, tagset_sizes, char_size, char_dim, char_hidden_dim, char_rnn_layers, embedding_dim, word_hidden_dim, word_rnn_layers, vocab_size, dropout_ratio, file_num, large_CRF=True, if_highway = False, in_doc_words = 2, highway_layers = 1):
        ...
        for i in range(file_num - 1):
            self.word_lstms_list.append(nn.LSTM(word_hidden_dim, word_hidden_dim // 2, num_layers=1, bidirectional=True, dropout=dropout_ratio))
    ...
    def forward(self, forw_sentence, forw_position, back_sentence, back_position, word_seq, file_no, hidden=None):
        ...
        #word level lstm
        word_lstm = self.word_lstms_list[file_no]
        lstm_out, _ = word_lstm(word_input)

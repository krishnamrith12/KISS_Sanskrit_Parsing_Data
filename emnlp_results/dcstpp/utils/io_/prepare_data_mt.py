...
def read_data(source_path, alphabets, max_size=None,
              lower_case=False, symbolic_root=False, symbolic_end=False, predictor=None):
    ...
    for path in source_path:
        reader = Reader(path, alphabets, predictor=predictor)
            ...
            for bucket_id, bucket_size in enumerate(_buckets):
                if inst_size < bucket_size:
                    data[bucket_id].append([sent.word_ids, sent.char_id_seqs, inst.ids['pos_alphabet'], inst.ids['ner_alphabet'],
                                            inst.heads, inst.ids['arc_alphabet'], inst.ids['auto_label_alphabet'], sent.word_list])
...
def read_data_to_variable(source_path, alphabets, device, max_size=None,
    ...
    data, max_char_length = read_data(source_path, alphabets,
                                      ...
                                      symbolic_root=symbolic_root, symbolic_end=symbolic_end, predictor=predictor)
    ...
    for bucket_id in range(len(_buckets)):
        ...
        for i, inst in enumerate(data[bucket_id]):
            wids, cid_seqs, pids, nids, hids, aids, mids, word_list = inst
            inst_size = len(wids)
            lengths[i] = inst_size
            # word ids
        ...
        #f_f, f_p, b_f, b_p, w_f, mask_v, file_no = predictor.encode_sentences(all_word_list, 4)
        #print(f_p.shape)
        #print(b_f.shape)
        #print(b_p.shape)
        #print(w_f.shape)
        #print(mask_v.shape)
        #print(wid_inputs.shape)
        #print(cid_inputs.shape)
        ...
        #f_f_concat = torch.cat()

        data_variable.append((words, chars, pos, ner, heads, arc, auto_label, masks, single, lengths, all_word_list))
...
def iterate_batch_rand_bucket_choosing(data, batch_size, device, unk_replace=0.0, ner_model=True, predictor=None):
    data_variable, bucket_sizes = data
    ...
    while sum(bucket_sizes) > 0:
        ...
        if ner_model:
            words, chars, pos, ner, heads, arc, auto_label, masks, single, lengths, all_word_list = data_variable[bucket_id]
        else:
            words, chars, pos, ner, heads, arc, auto_label, masks, single, lengths = data_variable[bucket_id]
        ...
        words_list_subset = [all_word_list[wl_idx] for wl_idx in list(set_indices)]
        f_f, f_p, b_f, b_p, w_f, mask_v, file_no = predictor.encode_sentences(words_list_subset, 4)
        #print('Here')
        #print(f_p.shape)
        #print(b_f.shape)
        #print(b_p.shape)
        ...
        if ner_model:
            #if torch.sum(mask_v[-1,:]).item() == 0:
            #    print(torch.sum(mask_v[-1,:]).item())
            #    raise Exception("sdsd")
            #yield words, chars[indices], pos[indices], ner[indices], heads[indices], arc[indices], auto_label[indices], masks[indices], lengths[indices], f_f[:, indices], f_p[:, indices], b_f[:, indices], b_p[:, indices], w_f[:, indices], mask_v[:, indices], file_no
            yield words, chars[indices], pos[indices], ner[indices], heads[indices], arc[indices], auto_label[indices], masks[indices], lengths[indices], f_f, f_p, b_f, b_p, w_f, mask_v, file_no
        else:

...
from utils.io_ import seeds, Writer, get_logger, rearrange_splits
from utils.io_ import  prepare_data_mt as prepare_data
...
import uuid
from multi_task_tagger import downstream_utils
...
def read_arguments():
    ...
    args_.add_argument('--use_multi_task', action='store_true', help='Use MultiTask Layers in Dependency Parser')
    args_.add_argument('--ner_model_same_projections', action='store_true', help='Use MultiTask Layers in Dependency Parser')
    args = args_.parse_args()
    args_dict = {}

    args_dict['use_multi_task'] = args.use_multi_task
    args_dict['ner_model_same_projections'] = args.ner_model_same_projections
...
def build_model_and_optimizer(args, ner_model=None):
    ...
    ner_model_separate_projections = not args.ner_model_same_projections
    model = BiAffine_Parser_Gated(args.word_dim, args.num_word, args.char_dim, args.num_char,
                            ...
                            biaffine=True, arc_decode=args.arc_decode, initializer=args.initializer, ner_model=ner_model, freeze_ner_model=False,
                            ner_model_separate_projections=ner_model_separate_projections)

    #if args.use_multi_task:
        # Use layers from pre-trained multitask tagger
...
def in_domain_evaluation(args, datasets, model, optimizer, dev_eval_dict, test_eval_dict, epoch,
                         best_model, best_optimizer, patient, ner_model=True, predictor=None):
    # In-domain evaluation
    #curr_dev_eval_dict = evaluation(args, datasets['dev'], 'dev', model, args.domain, epoch, 'current_results')
    curr_dev_eval_dict = evaluation(args, datasets['dev'], 'dev', model, args.domain, epoch, 'current_results', ner_model=ner_model, predictor=predictor)
    ...
    if is_best_in_domain:
        ...
        curr_test_eval_dict = evaluation(args, datasets['test'], 'test', model, args.domain, epoch, 'current_results', ner_model=ner_model, predictor=predictor)
...
def write_results(args, data, data_domain, split, model, model_domain, eval_dict, ner_model=True, predictor=None):
...
def main():
    ...
    if args.use_multi_task:
        # Load Predictor for MultiTask
        logger.info('**'*40)
        logger.info('**'*40)
        logger.info('**'*40)
        logger.info("Now loading the multitask model and multi task predictor ..... ")
        predictor_list, ner_model = downstream_utils.get_multi_task_model()
        logger.info('**'*40)
        logger.info('**'*40)
        logger.info('**'*40)
    else:
        logger.info('Not using multi task layers!')
        predictor_list = [None]
        ner_model = None
    ...
    for split in args.splits:
        dataset = prepare_data.read_data_to_variable(args.data_paths[split], args.alphabets, args.device,
                                                     symbolic_root=True, predictor=predictor_list[0])
    ...
    model, optimizer, dev_eval_dict, test_eval_dict, start_epoch = build_model_and_optimizer(args, ner_model=ner_model)
    best_model = deepcopy(model)
    best_optimizer = deepcopy(optimizer)
    ...
    if not args.eval_mode:
        ...
        for epoch in range(start_epoch + 1, args.num_epochs + 1):
            ...
            if ner_model is not None:
                train_iter = prepare_data.iterate_batch_rand_bucket_choosing(
                    datasets['train'], args.batch_size, args.device, unk_replace=args.unk_replace, ner_model=True, predictor=predictor_list[0])
            else:
                train_iter = prepare_data.iterate_batch_rand_bucket_choosing(
            ...
            for batch_num, batch in enumerate(train_iter):
                ...
                if ner_model is not None:
                    word, char, pos, ner_tags, heads, arc_tags, auto_label, masks, lengths, f_f, f_p, b_f, b_p, w_f, mask_v, file_no = batch
                    #print(lengths)
                    #print(masks.shape)
                    #print(masks)
                    #print(mask_v)
            ...
            dev_eval_dict, test_eval_dict, best_model, best_optimizer, patient = in_domain_evaluation(args, datasets, model, optimizer, dev_eval_dict, test_eval_dict, epoch, best_model, best_optimizer, patient, ner_model=ner_model, predictor=predictor_list[0])
        ...
        for split in datasets.keys():
            ...
            evaluation(args, datasets[split], split, best_model, args.domain, epoch, 'best_results', ner_model=ner_model, predictor=predictor_list[0])
        ...
        for split in ['train', 'dev', 'test']:
            ...
            eval_dict = evaluation(args, datasets[split], split, model, args.domain, epoch, 'best_results', ner_model=ner_model, predictor=predictor_list[0])
            write_results(args, datasets[split], args.domain, split, model, args.domain, eval_dict, ner_model=ner_model, predictor=predictor_list[0])

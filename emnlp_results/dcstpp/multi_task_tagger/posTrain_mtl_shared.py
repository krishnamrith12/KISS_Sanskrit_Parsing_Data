...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_files', nargs='+', default='./data/ner2003/eng.train.iobes', help='Apply output annotation to files')
    ...
    #file_prefix = 'sanskrit_data/pkl_files/200k_without_pos_'
    ...
    for epoch_idx, args.start_epoch in enumerate(epoch_list):
        ...
        for sample_id in tqdm( range(sample_num) , mininterval=2,
            ...
            print("Sampling from task : {0} - order is cas-gen-num".format(file_no))
        ...
        if 'f' in args.eva_matrix:
            ...
            if dev_acc > best_acc[file_no]:
                ...
                if args.output_annotation: #NEW
                    print('annotating')
                    with open(output_directory + 'output_best'+str(file_no)+'.txt', 'w') as fout:
                        predictor_list[file_no].output_batch(ner_model, test_word[file_no], fout, file_no)

...
class LM_LSTM_CRF(nn.Module):
    ...
    def __init__(self, tagset_sizes, char_size, char_dim, char_hidden_dim, char_rnn_layers, embedding_dim, word_hidden_dim, word_rnn_layers, vocab_size, dropout_ratio, file_num, pad_value, large_CRF=True, if_highway = False, in_doc_words = 2, highway_layers = 1, n_heads=4, d_model=128):
        ...
        self.word_lstm_task_0_self_attn = MultiHeadedAttention(n_heads, word_hidden_dim)
        ...
        for i in range(file_num - 1):
            ...
            self.word_lstms_list.append(nn.LSTM(word_input_size + 2 * word_hidden_dim, word_hidden_dim // 2, num_layers=1, bidirectional=True, dropout=dropout_ratio))
            self.lstm_self_attn_list.append(MultiHeadedAttention(n_heads, word_hidden_dim))
        ...
        for i in range(file_num):
            if large_CRF:
                self.crflist.append(crf.CRF_L(2 * word_hidden_dim, tagset_sizes[i]))
            else:
                self.crflist.append(crf.CRF_S(2 * word_hidden_dim, tagset_sizes[i]))
    ...
    def forward(self, forw_sentence, forw_position, back_sentence, back_position, word_seq, file_no, hidden=None):
        ...
        attn_context = self.self_attn(word_input_trans, word_input_trans ,word_input_trans, src_mask)
        ...
        for i in range(file_no + 1):
            ...
            lstm_output_trans = lstm_output.permute(1,0,2).float()
            lstm_atten_context = self.lstm_self_attn_list[i](lstm_output_trans, lstm_output_trans, lstm_output_trans, src_mask)
            lstm_atten_context = lstm_atten_context.permute(1,0,2)
            lstm_output = torch.cat([lstm_output, lstm_atten_context], dim=2)

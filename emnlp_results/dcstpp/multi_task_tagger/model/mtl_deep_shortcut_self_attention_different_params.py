...
class LM_LSTM_CRF(nn.Module):
    ...
    def __init__(self, tagset_sizes, char_size, char_dim, char_hidden_dim, char_rnn_layers, embedding_dim, word_hidden_dim, word_rnn_layers, vocab_size, dropout_ratio, file_num, pad_value, large_CRF=True, if_highway = False, in_doc_words = 2, highway_layers = 1, n_heads=4, d_model=128):
        ...
        self.self_attn = MultiHeadedAttention(n_heads, char_hidden_dim * 2 + embedding_dim)
        ...
        self.word_embeds = nn.Embedding(vocab_size, embedding_dim) # Will be used for query of self attention

        self.word_embeds_key = nn.Embedding(vocab_size, embedding_dim)
        self.word_embeds_value = nn.Embedding(vocab_size, embedding_dim)

        self.word_lstms_list = nn.ModuleList()

        word_input_size = embedding_dim + char_hidden_dim * 2 + self.d_model

        self.word_lstm_task_0 = nn.LSTM(embedding_dim + char_hidden_dim * 2 + self.d_model , word_hidden_dim // 2, num_layers=word_rnn_layers, bidirectional=True, dropout=dropout_ratio)
    ...
    def load_pretrained_word_embedding(self, pre_word_embeddings):
        ...
        self.word_embeds_key.weight = nn.Parameter(pre_word_embeddings)
        self.word_embeds_value.weight = nn.Parameter(pre_word_embeddings)
    ...
    def rand_init(self, init_char_embedding=True, init_word_embedding=False):
        ...
        if init_word_embedding:
            ...
            utils.init_embedding(self.word_embeds_key.weight)
            utils.init_embedding(self.word_embeds_value.weight)
        if self.if_highway:
        ...
        for word_lstm in self.word_lstms_list:
            utils.init_lstm(word_lstm)
    ...
    def forward(self, forw_sentence, forw_position, back_sentence, back_position, word_seq, file_no, hidden=None):
        ...
        word_emb_key = self.word_embeds_key(word_seq)
        word_emb_value = self.word_embeds_value(word_seq)

        word_seq_trans = word_seq.transpose(1,0).float()
        # Applying self atttention
        src_mask = (word_seq_trans != self.pad_value).unsqueeze(-2).float()

        d_word_emb = self.dropout(word_emb)
        d_word_emb_key = self.dropout(word_emb_key)
        d_word_emb_value = self.dropout(word_emb_value)

        #combine
        word_input = torch.cat((d_word_emb, d_char_out), dim = 2)
        word_input_key = torch.cat((d_word_emb_key, d_char_out), dim = 2)
        word_input_value = torch.cat((d_word_emb_value, d_char_out), dim = 2)


        word_input_trans = word_input.permute(1,0, 2).float()
        word_input_trans_key = word_input_key.permute(1,0, 2).float()
        word_input_trans_value = word_input_value.permute(1,0, 2).float()

        attn_context = self.self_attn(word_input_trans, word_input_trans_key ,word_input_trans_value, src_mask)
        attn_context = attn_context.permute(1,0,2)
        word_input = torch.cat([word_input, attn_context], dim=2)
        ...
        crf_out = crf_out.view(self.word_seq_length, self.batch_size, self.tagset_sizes[file_no], self.tagset_sizes[file_no])

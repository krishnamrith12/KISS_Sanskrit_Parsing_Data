...
def read_corpus(lines):
    ...
    for line in lines:
        ...
        #else:
        #    text_arr = text.split()
        #    tmp_fl.append(text_arr[0])
        #    tmp_ll.append(text_arr[-1])
        if not (line.isspace() or (len(line) > 10 and line[0:10] == '-DOCSTART-')):
            line = line.rstrip('\n').split()
            tmp_fl.append(line[0])
            tmp_ll.append(line[-1])
    ...
    if len(tmp_fl) > 0:
        features.append(tmp_fl)
        labels.append(tmp_ll)
    #print("Number of sentences loaded : " + str(len(features)))
...
def read_features_sentences(lines):
...
def create_word_dict(feature_map):
...
def load_embedding_wlm(emb_file, delimiter, feature_map, full_feature_set, caseless, unk, emb_len, shrink_to_train=False, shrink_to_corpus=False):
    ...
    if emb_file.endswith('.txt'):
        ...
        rand_embedding_tensor, indoc_embedding_array, indoc_word_array, outdoc_embedding_array, outdoc_word_array = load_embeddings_from_txt_file(emb_file, delimiter, shrink_to_train, feature_set, unk, rand_embedding_tensor, word_dict, full_feature_set, indoc_embedding_array, indoc_word_array, shrink_to_corpus, outdoc_embedding_array, outdoc_word_array)

    else:
        rand_embedding_tensor, indoc_embedding_array, indoc_word_array, outdoc_embedding_array, outdoc_word_array = load_embeddings_from_bin_file(emb_file, delimiter, shrink_to_train, feature_set, unk, rand_embedding_tensor, word_dict, full_feature_set, indoc_embedding_array, indoc_word_array, shrink_to_corpus, outdoc_embedding_array, outdoc_word_array)
...
def load_embeddings_from_txt_file(emb_file, delimiter, shrink_to_train, feature_set, unk, rand_embedding_tensor, word_dict, full_feature_set, indoc_embedding_array, indoc_word_array, shrink_to_corpus, outdoc_embedding_array, outdoc_word_array):
    ...
    for line in open(emb_file, 'r'):
        ...
        elif line[0] in full_feature_set: # All words not in train set but in dev or test set
            indoc_embedding_array.append(vector)
            indoc_word_array.append(line[0])
        elif not shrink_to_corpus: # All words neither in train, dev or test set
            outdoc_word_array.append(line[0])
            outdoc_embedding_array.append(vector)

    return rand_embedding_tensor, indoc_embedding_array, indoc_word_array, outdoc_embedding_array, outdoc_word_array



def load_embeddings_from_bin_file(emb_file, delimiter, shrink_to_train, feature_set, unk, rand_embedding_tensor, word_dict, full_feature_set, indoc_embedding_array, indoc_word_array, shrink_to_corpus, outdoc_embedding_array, outdoc_word_array):

    print('Embedding file is a binary file...have to use gensim to get embeddings')

    # Temporary fix for utf8 codec error, find permanent fix TODO
    wv_model = gensim.models.KeyedVectors.load_word2vec_format(emb_file, binary=True, unicode_errors='ignore')
    vocab_words = []
    for word in wv_model.index2word:
        vocab_words.append(word)
    for word in vocab_words:
        if shrink_to_train and word not in feature_set:
            continue
        if word == unk:
            rand_embedding_tensor[0] = torch.FloatTensor(wv_model[word])
        elif word in word_dict:
            rand_embedding_tensor[word_dict[word]] = torch.FloatTensor(wv_model[word])
        elif word in full_feature_set:
            indoc_embedding_array.append(wv_model[word])
            indoc_word_array.append(word)
        elif not shrink_to_corpus: # All words neither in train, dev or test set
            outdoc_embedding_array.append(wv_model[word])
...
def construct_bucket_mean_vb_wc(word_features, input_label, label_dict, char_dict, word_dict, caseless):
    ...
    print('Encoding words done.')
    print('Going to call function construct_bucket_vb_wc(), which Constructs bucket by thresholds for viterbi decode, word-level and char-level')
    print(label_dict)
...
def construct_bucket_vb_wc(word_features, forw_features, fea_len, input_labels, thresholds, pad_word_feature, pad_char_feature, pad_label, label_size):
    ...
    print('Step - 1 of the bucketing done.')
    print(buckets_len)
    # calc padding
    for f_f, f_l, w_f, i_l in zip(forw_features, fea_len, word_features, input_labels):
        ...
        #print(w_f)
        #print(i_l)
        #print(buckets[idx][0][0])
        #print(buckets[idx][1][0])
        #print(buckets[idx][4][0])
        #print(buckets[idx][5][0])
        #print(buckets[idx][6][0])
        #print(buckets[idx][7][0])
    ...
    print('Bucketing complete. Returning.')
...
def save_data_pkl_file(dataset, forw, back, filename):
...
def save_dict_pkl_file(dic, filename):

    with open(filename, 'wb') as fp:
        pickle.dump(dic, fp)
def load_data_pkl_file(filename):

    print("Loading bucketed dataset from pkl file {0}".format(filename))

    with open(filename, 'rb') as fp:
        d = pickle.load(fp)

    return d['dataset'], d['forw'], d['back']

def load_label_set_from_file(filename):
...
def save_embeddings(ner_model, documents, batch_size=50):
    ...
    for d_ind in tqdm( range(0, d_len), mininterval=1,
        ...
        for ind in range(0, f_len, batch_size):
            eind = min(f_len, ind + batch_size)

...
class BiAffine_Parser_Gated(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, use_pos, use_char, pos_dim, num_pos, num_filters,
                 ...
                 biaffine=True, arc_decode='mst', initializer=None, ner_model=None, project_ner_model=True, ner_model_separate_projections=True, freeze_ner_model=True, layers_to_use=[0, 1, 2, 3, 4], use_morph=[0, 1, 2, 3]):
        super(BiAffine_Parser_Gated, self).__init__()
        self.num_gates = num_gates
        self.ner_model = ner_model
        self.input_dim = 2 * hidden_size # Input Dim for each of the gates, should be same for all encoders
        self.ner_model_separate_projections = ner_model_separate_projections

        self.use_morph = use_morph

        print('ner_model_separate_projections : ' + str(ner_model_separate_projections))

        self.project_ner_model = project_ner_model
        if self.ner_model is not None:
            self.num_ner_layers = len(self.ner_model.word_lstms_list)
            self.ner_lstm_word_hidden_size = self.ner_model.word_hidden_dim
        else:
            self.num_ner_layers = 0
            self.ner_lstm_word_hidden_size = 0

        if project_ner_model:
            self.num_gates = self.num_gates + self.num_ner_layers

        if project_ner_model:
            if ner_model_separate_projections:
                self.ner_projections = nn.ModuleList()
                for i in range(self.num_ner_layers):
                    self.ner_projections.append(nn.Linear(self.ner_lstm_word_hidden_size, self.input_dim))
            else:
                self.ner_projections = nn.Linear(self.ner_lstm_word_hidden_size, self.input_dim)
        elif self.ner_lstm_word_hidden_size != self.input_dim:
            raise ValueError('Hidden Dim of Multi Task LSTM should be same as that of BiAffine_Parser_Gated. {}|{}'.format(self.ner_lstm_word_hidden_size, self.input_dim))
        else:
            raise NotImplementedError('Not implemented. No such mode')

        if ner_model is not None and freeze_ner_model:
            print('Freezing the ner_model')
        ...
        self.rnn_encoder = BiRecurrentConv_Encoder(word_dim, num_words, char_dim, num_chars, use_pos, use_char,
                                                   ...
                                                   p_in=p_in, p_out=p_out, p_rnn=p_rnn, initializer=initializer, use_morph)
        if self.num_gates >= 2:
            num_gates_without_ner_model = self.num_gates - self.num_ner_layers
            self.extra_rnn_encoders = nn.ModuleDict([[str(i), BiRecurrentConv_Encoder(word_dim, num_words, char_dim, num_chars, use_pos, use_char,
                                                   ...
                                                   num_layers, embedd_word=embedd_word,
                                                   embedd_char=embedd_char, embedd_pos=embedd_pos,
                                                   p_in=p_in, p_out=p_out, p_rnn=p_rnn, initializer=initializer)] for i in range(num_gates_without_ner_model - 1)])
            self.gate = Gating(self.num_gates, 2 * hidden_size)
        else:
    ...
    def forward(self, input_word, input_char, input_pos, f_f=None, f_p=None, b_f=None, b_p=None, w_f=None, mask_v=None, file_no=None, mask=None, length=None, hx=None):
        ...
        if self.num_gates >= 2:
            ...
            if self.ner_model is not None:
                ner_lstms_outs = []
                ner_model_lstms_outputs = self.ner_model(f_f, f_p, b_f, b_p, w_f, file_no, return_lstm_outputs_only=True)
                for i in range(len(ner_model_lstms_outputs)):
                    if self.ner_model_separate_projections:
                        extra_enconder_outputs.append(self.ner_projections[i](ner_model_lstms_outputs[i].transpose(0,1)))
                    else:
                        extra_enconder_outputs.append(self.ner_projections(ner_model_lstms_outputs[i].transpose(0,1)))
...
class BiRecurrentConv_Encoder(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, use_pos, use_char, pos_dim, num_pos, num_filters,
            ...
            p_in=0.33, p_out=0.33, p_rnn=(0.33, 0.33), initializer=None, use_morph=[0, 1, 2, 3]):
        ...
        # 0: num, 1: gen, 2: cas, 3: Tense
        # Morpholoical tags
        self.morph_embedds = nn.ModuleList()
        if len(use_morph) > 0:
            self.morph_embedds.append(Embedding(num_pos, pos_dim))

        print('No. of Morph tags ' + str(len(use_morph)))
        print(use_morph)
        ...
        if len(use_morph) > 0:
            self.dim_enc += len(use_morph) * pos_dim
    ...
    def forward(self, input_word, input_char, input_pos, input_morph=None, mask=None, length=None, hx=None):
        ...
        morphs = []
        if len(self.use_morph) > 0 and input_morph is not None and len(input_morph) > 0:
            for i in range(len(input_morph)):
                mo = self.morph_embedds[i](input_morph[i])
                mo = self.dropout_in(mo)
                morphs.append(mo)
            mo = torch.cat(morphs, dim=2)
            input = torch.cat([input, mo], dim=2)
